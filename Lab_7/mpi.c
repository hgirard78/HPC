...
int main (int argc, char *argv[])
{
    ...
    if (rank == 0)
    {
        ...
        for (dest=1; dest<=numworkers; dest++)
        {
            ...
            {
                MPI_Send(&a[i], N, MPI_DOUBLE, dest, 1, MPI_COMM_WORLD);
            }
            offset = offset + rows;
        }
        ...
        for (i=1; i<=numworkers; i++)
        {
            ...
            for (j = offset; j < offset + rows; j++)
                MPI_Recv(&c[j], N, MPI_DOUBLE, source, 1, MPI_COMM_WORLD, &status);
    ...
    {      
        ...
        for (j = offset; j < offset + rows; j++)
            MPI_Recv(&a[j], N, MPI_DOUBLE, 0, 1, MPI_COMM_WORLD, &status);
        ...
        for (dest = 1; dest <= numworkers; dest++)
        {
            ...
            for (i = offset; i < offset + rows; i++)
            {
                MPI_Send(&c[i], N, MPI_DOUBLE, dest, 1, MPI_COMM_WORLD);
            }
        }
